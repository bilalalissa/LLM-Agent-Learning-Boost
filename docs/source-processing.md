...
Older source pages are also repaired at startup by the source-section backfill. When an existing `wiki/sources/*.md` page lacks a `## Learning Boost` section, the app derives one from that page’s Summary, Key Points, links, and open questions while preserving `## User Notes`.
